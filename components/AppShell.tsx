    ...
    <main className="wuxianpi-app chat-mode shell-chat-default">
      ...
      <aside className={`shell-drawer shell-drawer-l1 ${l1Open ? "open" : ""}`} aria-hidden={!l1Open} aria-label="主菜单">
        ...
        <footer className="shell-drawer-footer">
          <small>v{process.env.NEXT_PUBLIC_APP_VERSION ?? "0.1.0"} · Pi {process.env.NEXT_PUBLIC_PI_VERSION ?? "?"}</small>
      ...
      <div className="settings-stack">
        ...
        <section className="settings-card">
          <header>
            <div>
              <strong>运行信息</strong>
              <small>WuxianPi v{process.env.NEXT_PUBLIC_APP_VERSION ?? "0.1.0"} · Pi v{process.env.NEXT_PUBLIC_PI_VERSION ?? "unknown"}</small>
            </div>
