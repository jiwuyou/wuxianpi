...
function PiAgentTitle() {
  ...
  const target = showVersion ? `${process.env.NEXT_PUBLIC_APP_VERSION ?? "0.0.0"}p${process.env.NEXT_PUBLIC_PI_VERSION ?? "0.0.0"}` : "Pi MyWeb";
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <button
              ...
              title={selectedCwd ? `在 ${selectedCwd} 新建会话` : "请先选择项目"}
              onMouseEnter={(e) => {
                if (!selectedCwd) return;
        ...
        <div ref={dropdownRef} style={{ position: "relative" }}>
          ...
          >
            <span
              ...
              title={selectedCwd ?? ""}
            >
              {selectedCwd ? shortenCwd(selectedCwd, homeDir) : (initialSessionId && !restoredRef.current ? "" : "选择项目...")}
            </span>
          </button>
