...
export const MCP_SERVER_SCHEMA = {
  oneOf: [
    ...
    },
    {
      type: "object",
      additionalProperties: false,
      required: ["id", "name", "transport", "url"],
      properties: {
        ...mcpBaseProperties(),
        transport: { const: "streamable-http" },
      },
      not: { anyOf: [{ required: ["command"] }, { required: ["args"] }, { required: ["cwd"] }] },
    },
  ],
} as const;

function mcpBaseProperties() {
  const stringRecord = { type: "object", additionalProperties: { type: "string" } } as const;
  return {
    ...
    env: stringRecord,
    envSecretRefs: stringRecord,
    headers: stringRecord,
    headerSecretRefs: stringRecord,
    timeoutMs: { type: "number", exclusiveMinimum: 0 },
    lifecycle: { enum: ["keep-alive", "lazy", "lazy-keep-alive", "eager"] },
    auth: { enum: ["oauth", "bearer", false] },
    enabled: { type: "boolean" },
  } as const;
}
